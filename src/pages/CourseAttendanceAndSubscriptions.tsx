      ...
      <div className="relative overflow-hidden bg-gradient-to-r from-teal-700 via-emerald-600 to-blue-700 rounded-3xl p-6 sm:p-8 shadow-xl text-white border border-teal-500/20">
        ...
        <div className="relative z-10 space-y-2">
          <span className="px-3.5 py-1 bg-white/25 backdrop-blur-md text-white text-xs font-bold rounded-full inline-flex items-center gap-1.5 border border-white/30">
            <FolderTree size={13} />
            سجلات اختبارات الطلاب - منصة Z E D
          </span>
          <h1 className="text-2xl sm:text-3xl font-black tracking-wide">
            إدارة كافة محاولات واختبارات الطلاب
          </h1>
          <p className="text-xs sm:text-sm text-teal-100 max-w-xl leading-relaxed">
            يتم عرض كافة الاختبارات والمحاولات التي أجراها الطلاب بشكل مستقل لتتمكن من متابعة كل اختبار على حدة.
          </p>
        </div>
      </div>
              ...
              >
                <div className="space-y-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-500 mt-1">انقر لمتابعة جميع اختبارات ومحاولات الطلاب</p>
                  </div>
                </div>
                <div className="pt-2 flex items-center justify-between text-xs font-bold text-teal-600 border-t border-slate-100">
                  <span>استعراض سجلات الاختبارات</span>
                  <ChevronLeft size={16} />
                </div>
              </div>
            ))}
      ...
      {selectedSpecialty && selectedCourse && (
        <div className="bg-white border-2 border-teal-100 rounded-3xl p-6 sm:p-8 shadow-xs space-y-6">
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4 pb-4 border-b border-teal-100">
            <div>
              <h3 className="text-base font-bold text-slate-900">
                سجلات اختبارات كورس {selectedCourse} ({filteredStudents.length} سجل/محاولة)
              </h3>
              <span className="text-xs text-slate-500">تظهر هنا كافة الاختبارات والمحاولات التي قام الطلاب بحلها بشكل منفصل</span>
            </div>
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2.5">
                      ...
                      <h4 className="text-sm font-black text-slate-900">{record.student_name}</h4>
                    </div>
                    <button
                  ...
                  {/* اسم أو تفاصيل الاختبار إن وجدت في الجدول */}
                  <div className="bg-white px-3 py-2 rounded-xl border border-slate-200 flex items-center gap-2 text-xs font-bold text-slate-700">
                    <FileText size={14} className="text-teal-600 shrink-0" />
                    <span className="truncate">الاختبار: {record.quiz_name || record.exam_title || record.course_name}</span>
                  </div>

                  <div className="flex items-center justify-between pt-1">
                    <span className="text-[11px] text-slate-500 font-semibold">حالة الحضور:</span>
                    <button
                      onClick={() => handleToggleAttendance(record.id, record.attendance_status)}
                      className={`px-3 py-1.5 rounded-xl text-xs font-bold flex items-center gap-1.5 transition-all shadow-2xs ${
                        record.attendance_status === "حاضر"
                          ? "bg-emerald-600 hover:bg-emerald-700 text-white"
                          : "bg-rose-600 hover:bg-rose-700 text-white"
                      }`}
                    >
                      {record.attendance_status === "حاضر" ? <CheckCircle2 size={13} /> : <XCircle size={13} />}
                      <span>{record.attendance_status} (تغيير)</span>
                    </button>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 gap-2.5 pt-3 border-t border-slate-200/60 text-xs">
                  <div className="bg-white p-2.5 rounded-xl border border-slate-200 flex items-center justify-between">
                    <span className="text-slate-400 font-semibold flex items-center gap-1">
                      <Calendar size={13} className="text-teal-600" /> وقت الحل والتسجيل:
                    </span>
                    <span className="font-bold text-slate-700">{record.attendance_time || "-"}</span>
                  </div>
